...
def generate_main(prefix, industry, length, nsamples):
    '''parser = argparse.ArgumentParser()
    parser.add_argument('--device', default='0,1,2,3',
                        type=str, required=False, help='生成设备')
    ...
    parser.add_argument('--model_config', default='config/model_config_small.json', type=str, required=False,
                        help='模型参数')
    parser.add_argument('--tokenizer_path', default='cache/vocab_mk_44MB.txt',
                        type=str, required=False, help='词表路径')
    parser.add_argument('--model_path', default='model/final_model',
                        type=str, required=False, help='模型路径')
    parser.add_argument('--prefix', default='萧炎', type=str,
                        required=False, help='生成文章的开头')
    parser.add_argument(
    ...
    parser.add_argument('--save_samples_path', default='.',
                        type=str, required=False, help="保存样本的路径")
    ...
    # if args.segment:
    from GPT2_Chinese_master.tokenizations import tokenization_bert_word_level as tokenization_bert
    # else:
    #from tokenizations import tokenization_bert

    os.environ["CUDA_VISIBLE_DEVICES"] ='0,1,2,3' #args.device  # 此处设置程序使用哪些显卡
    #length = args.length
    batch_size = 1  # args.batch_size
    #nsamples = args.nsamples
    temperature = 1  # args.temperature
    topk = 8  # args.topk
    topp = 0  # args.topp
    ...
    if industry=='makeup':
        tokenizer = tokenization_bert.BertTokenizer(
            vocab_file='GPT2_Chinese_master/cache/vocab_mk_44MB.txt')  # args.tokenizer_path)
        model = GPT2LMHeadModel.from_pretrained(
            'GPT2_Chinese_master/model/final_makeup_model')  # args.model_path)
    elif industry=='education':
        tokenizer = tokenization_bert.BertTokenizer(
            vocab_file='GPT2_Chinese_master/cache/vocab_education_37MB.txt')  # args.tokenizer_path)
        model = GPT2LMHeadModel.from_pretrained(
            'GPT2_Chinese_master/model/final_education_model')  # args.model_path)
    model.to(device)
    model.eval()
    ...
    while True:
        ...
        for _ in range(nsamples // batch_size):
            out = generate(
                ...
                length=length,
                # args.fast_pattern, tokenizer=tokenizer,
